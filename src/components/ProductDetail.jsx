...
import { Star, ShieldCheck, ShoppingCart, ArrowLeft, RefreshCw, Heart, Share2, Scale, X } from 'lucide-react';
...
export default function ProductDetail({ product, onBack }) {
  ...
  // 🚀 FIX: Disable body scroll when ProductDetail opens (like a modal)
  useEffect(() => {
    ...
    // Cleanup function: Re-enable scroll when component unmounts
    return () => {
  ...
  return (
    <>
      {/* 🚀 FIX: Overlay Background - Modal Style */}
      <div className="fixed inset-0 bg-black/50 z-40" onClick={handleBack}></div>
      
      {/* Product Detail Modal Container */}
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
        <div className="bg-white rounded-2xl max-w-[1200px] w-full max-h-[90vh] overflow-y-auto relative shadow-2xl">
          
          {/* Close Button - Top Right */}
          <button 
            onClick={handleBack}
            className="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-white/80 hover:bg-white shadow-md flex items-center justify-center transition-all hover:scale-105"
          >
            <X className="w-5 h-5 text-neutral-700" />
          </button>

          {/* Main Content */}
          <div className="py-6 px-4 sm:px-6 select-none font-sans">
            
            {/* Breadcrumb Navigation Path */}
            <div className="flex items-center gap-2 text-xs text-neutral-400 mb-6 tracking-wide">
              <button onClick={handleBack} className="hover:text-black transition-colors flex items-center gap-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
              ...
              <div className="md:col-span-6 grid grid-cols-12 gap-4">
                ...
                {/* Large display box */}
                <div className="col-span-10 bg-[#f8f8f8] aspect-square flex items-center justify-center p-8 relative rounded-xl">
                  <div className="w-[90%] h-[90%] flex items-center justify-center">
                    <img src={activeImg} alt="Focus View" className="max-w-full max-h-full object-contain mix-blend-multiply" />
                  </div>
                  {product.discount && (
                    <span className="absolute top-4 left-4 bg-[#111111] text-white text-[11px] font-bold px-2 py-0.5 rounded-full">
                      {product.discount}
                    </span>
                  )}
                </div>
              ...
              <div className="md:col-span-6 flex flex-col space-y-5">
                <div>
                  ...
                  <p className="text-red-600 font-medium text-xs bg-red-50 border border-red-100 px-3 py-2 rounded-full inline-flex items-center gap-2">
                    <span className="w-2 h-2 bg-red-500 rounded-full animate-ping" />
                    Hurry up! {product.salesText || "Best Seller"}
                  </p>
                    ...
                    <div className="flex gap-2">
                      {['6', '7', '8', '9'].map((sz) => (
                        <button
                          ...
                          className={`h-9 w-10 text-xs font-semibold border rounded-full transition-all ${
                ...
                <div className="space-y-4 pt-4 border-t border-neutral-100">
                  <div className="flex gap-3">
                    ...
                    <button className="flex-1 bg-neutral-900 hover:bg-black text-white h-12 text-xs font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2 rounded-full shadow-md">
                      <ShoppingCart className="w-4 h-4" /> Add To Cart
                    </button>
                  </div>

                  {/* Buy Now Full Button */}
                  <button className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white h-12 text-xs font-bold uppercase tracking-widest transition-all rounded-full shadow-md">
                    Buy Now
                  </button>
                  ...
                  <div className="bg-neutral-50 p-4 border border-neutral-200 rounded-2xl space-y-2 mt-4 text-xs text-neutral-600">
