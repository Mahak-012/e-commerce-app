...
export default function PromoAndBrands({ onProductSelect }) {
  ...
  return (
    <section className="w-full select-none overflow-hidden relative">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-amber-50/40 via-orange-50/30 to-rose-50/40"></div>

      {/* Glowing orbs */}
      <div className="absolute top-[10%] left-[15%] w-[500px] h-[500px] bg-amber-200/25 rounded-full blur-[140px]"></div>
      <div className="absolute bottom-[5%] right-[20%] w-[400px] h-[400px] bg-rose-200/20 rounded-full blur-[120px]"></div>
      <div className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-orange-100/20 rounded-full blur-[160px]"></div>
      <div className="absolute top-[5%] right-[10%] w-[300px] h-[300px] bg-yellow-100/20 rounded-full blur-[100px]"></div>
      <div className="absolute bottom-[15%] left-[5%] w-[350px] h-[350px] bg-pink-100/15 rounded-full blur-[110px]"></div>
            ...
            <div className="flex w-[300%] animate-marquee whitespace-nowrap items-center gap-5 lg:gap-7">
              {continuousList.map((card, idx) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-neutral-900/75 via-neutral-900/20 to-transparent"></div>

                  {/* Warm Glow on Hover */}
                  <div className="absolute inset-0 bg-gradient-to-t from-amber-600/0 to-amber-400/0 group-hover:from-amber-600/10 group-hover:to-amber-400/5 transition-all duration-500"></div>

                  {/* Discount Badge */}
                  {card.discount && (
                    <div className="absolute top-4 left-4 bg-white text-neutral-900 text-[10px] font-bold px-3 py-1 rounded-full shadow-lg tracking-wider uppercase opacity-0 group-hover:opacity-100 -translate-y-2 group-hover:translate-y-0 transition-all duration-400">
                      {card.discount}
                    </div>
                  )}

                  {/* Brand Tag */}
                  <div className="absolute top-4 right-4 bg-white/15 backdrop-blur-md text-white text-[9px] font-semibold px-3 py-1 rounded-full border border-white/15 tracking-[0.2em] uppercase">
                    {card.brand}
                  </div>
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-5 lg:p-6">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      {/* CTA Arrow */}
                      <div className="w-9 h-9 rounded-full bg-white/15 backdrop-blur-sm border border-white/20 flex items-center justify-center opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all duration-400">
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-6 items-center justify-items-center">
              {brandLogos.map((brand, i) => (
                ...
                >
                  ...
                  />
                  <span
                    className="font-serif text-lg font-medium text-neutral-400 tracking-wider hidden"
                  >
                    {brand.name}
                  </span>
                </div>
